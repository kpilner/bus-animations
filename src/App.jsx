...
function App() {
  ...
  const [url, setUrl] = useState('/map.svg');
  ...
  // Facts position (px). X controls horizontal offset (margin-left), Y controls top.
  const [factsX, setFactsX] = useState(-580); // px
      ...
      <div className={styles.content}>
        ...
        <div className={styles.sidebar}>
          <div className={styles.editorPanel}>
            ...
            <label className={styles.editorField}>Facts (one per line)
              <textarea value={factsInput} onChange={(e)=>setFactsInput(e.target.value)} rows={6} />
            </label>
            <div style={{display:'flex', gap:'0.5rem'}}>
              <label className={styles.editorField} style={{flex:'1 1 0%'}}>Facts X (px)
                <input type="number" value={factsX} onChange={(e)=>setFactsX(parseInt(e.target.value)||0)} />
              </label>
              <label className={styles.editorField} style={{flex:'1 1 0%'}}>Facts Y (px)
